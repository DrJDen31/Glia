...
*/
Neuron::Neuron(const std::string id, const int complexity, const float resting, const float balancer = 1, const int refractory = 0, const int threshold = 0, const bool tick = false)
{
    this->id = id;
    this->value = resting;
...
void Neuron::receive(float transmission)
{
    ...
    if (this->value < this->threshold || this->using_tick)
    {
        return;
    }
